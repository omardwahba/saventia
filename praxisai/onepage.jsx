/* PraxisAI — single-page composition. All content as anchored sections. */
(function () {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 22, textAlign: align, alignItems: align === 'center' ? 'center' : 'flex-start' }}>
        ...
        <Reveal delay={240} style={{ display: 'flex', flexWrap: 'wrap', gap: 14, marginTop: 6, justifyContent: align === 'center' ? 'center' : 'flex-start' }}>
          <Button variant="primary" onClick={() => scrollTo('contact')}>{t.hero.primary} <Icon name="arrow-right" size={17} color="#FFFFFF" /></Button>
    ...
    return (
      <Reveal delay={200} style={{ position: 'relative' }}>
        <div style={{ position: 'relative', borderRadius: 'var(--radius-xl)' }}>
          ...
          <div aria-hidden="true" style={{ position: 'absolute', inset: '-12% -10% auto auto', width: 180, height: 180, background: 'radial-gradient(circle, rgba(37,99,235,0.4), transparent 70%)', filter: 'blur(20px)', zIndex: -1 }} />
        </div>
      </Reveal>
    );
  }
      ...
      <section id="accueil" style={{ position: 'relative', minHeight: '100svh', marginTop: -92, display: 'flex', alignItems: 'flex-end', overflow: 'hidden', scrollMarginTop: 104 }}>
        <div ref={imgRef} aria-hidden="true" style={{ position: 'absolute', inset: '-8% 0 0 0', zIndex: 0, willChange: 'transform' }}>
          {useArt
            ...
            : <window.PhotoSlot id="praxisai-hero-bleed" alt={t.locale === 'en' ? 'Add a high-end, human-centered photo with subtle AI/data elements' : 'Ajoutez une photo professionnelle, centrée sur l’humain, avec de subtils éléments IA/données'} radius={0} style={{ width: '100%', height: '108vh' }} />}
        </div>
  ...
  function Field({ label, name, type = 'text', value, onChange, error, placeholder, textarea }) {
    ...
    const common = {
      ...
      ...(focus ? { borderColor: 'var(--emerald-500)', boxShadow: '0 0 0 3px rgba(37,99,235,0.18)' } : {}),
    };
    return (
  ...
  function ContactForm({ t }) {
    ...
    if (sent) {
      return (
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', gap: 16, padding: '36px 8px' }}>
          <span style={{ display: 'grid', placeItems: 'center', width: 60, height: 60, borderRadius: '50%', background: 'rgba(37,99,235,0.16)', border: '1px solid rgba(37,99,235,0.4)' }}>
    ...
    return (
      <form onSubmit={submit} noValidate style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
        ...
        <Button variant="primary" type="submit" full>{c.submit} <Icon name="arrow-right" size={17} color="#FFFFFF" /></Button>
      </form>
    );
  }
  ...
  function OnePage({ t, scrollTo, heroLayout, visual }) {
    ...
    return (
      <React.Fragment>
        ...
        <Section pad="clamp(48px, 7vw, 104px)" {...anchor('approche')}>
          <Container>
            ...
            <div style={{ position: 'relative', marginTop: 40 }}>
              <div aria-hidden="true" className="method-line" style={{ position: 'absolute', top: 38, left: '12%', right: '12%', height: 1, background: 'linear-gradient(90deg, transparent, rgba(37,99,235,0.5), transparent)' }} />
        ...
        <Section pad="clamp(48px, 7vw, 96px)">
          <Container>
            <Reveal><SectionHeader eyebrow={t.values.eyebrow} title={t.values.title} align="center" /></Reveal>
            <div className="cards-5" style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 18, marginTop: 40 }}>
              {t.values.items.map((v, i) => (
                <Reveal key={i} delay={i * 60} style={{ height: '100%' }}>
                  <GlassCard pad={24} style={{ display: 'flex', flexDirection: 'column', gap: 12, height: '100%' }}>
                    <span style={{ display: 'grid', placeItems: 'center', width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, rgba(37,99,235,0.18), rgba(45,107,255,0.10))', border: '1px solid var(--glass-border)' }}>
                      <Icon name={v.icon} size={20} color="var(--emerald-400)" stroke={1.7} />
                    </span>
                    <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 500, fontSize: '1.08rem', color: 'var(--text-primary)', margin: 0, letterSpacing: '-0.01em' }}>{v.title}</h3>
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', lineHeight: 1.55, margin: 0 }}>{v.desc}</p>
                  </GlassCard>
                </Reveal>
              ))}
            </div>
              ...
              {t.industries.list.map((s, i) => (
                <Reveal key={i} delay={i * 50} style={{ height: '100%' }}>
                  <GlassCard pad={24} style={{ display: 'flex', flexDirection: 'column', gap: 14, height: '100%' }}>
                    <span style={{ display: 'grid', placeItems: 'center', width: 44, height: 44, borderRadius: 12, background: 'linear-gradient(135deg, rgba(37,99,235,0.16), rgba(45,107,255,0.10))', border: '1px solid var(--glass-border)' }}>
        ...
        <Section pad="clamp(48px, 7vw, 104px)" {...anchor('perspectives')}>
          <Container>
            <Reveal style={{ display: 'flex', flexWrap: 'wrap', gap: 16, alignItems: 'flex-end', justifyContent: 'space-between' }}>
              ...
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '7px 14px', borderRadius: 999, background: 'rgba(37,99,235,0.12)', border: '1px solid rgba(37,99,235,0.3)', color: 'var(--emerald-400)', fontSize: '0.82rem', fontWeight: 600 }}>
                <Icon name="info" size={15} color="var(--emerald-400)" /> {t.insights.note}
              </span>
            </Reveal>
            <div className="cards-3" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 22, marginTop: 40 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                  {[
                    ...
                    <div key={i} style={{ display: 'flex', gap: 14, alignItems: 'center' }}>
                      <span style={{ display: 'grid', placeItems: 'center', width: 42, height: 42, borderRadius: 12, background: 'linear-gradient(135deg, rgba(37,99,235,0.16), rgba(45,107,255,0.10))', border: '1px solid var(--glass-border)', flexShrink: 0 }}>
