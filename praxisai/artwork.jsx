/* On-brand SVG artifacts for image slots — Aurora Glass visual language.
   ...
   PraxisAI mark (leaf-flame + orbit) and a multidisciplinary constellation. */
(function () {
  let _u = 0;
  const uid = (p) => p + (++_u);
        ...
        <svg viewBox="0 0 420 520" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" aria-hidden="true" style={{ display: 'block' }}>
          <defs>
            ...
            <radialGradient id={g + '-orb'} cx="0.5" cy="0.5" r="0.5">
              ...
              <stop offset="0.6" stopColor="rgba(37,99,235,0.14)" />
          ...
          {/* gradient orb + the PraxisAI "P": continuous line, connected nodes, forward node */}
          <circle cx={cx} cy={cy} r="62" fill={`url(#${g}-orb)`} />
          <g transform={`translate(${cx} ${cy}) scale(3.1) translate(-29 -25)`} filter={`url(#${g}-glow)`}>
            <line x1="32" y1="18" x2="41.5" y2="18" stroke="var(--emerald-400)" strokeWidth="2.4" strokeLinecap="round" opacity="0.95" />
            <path d="M16 41 L16 9 Q 32 9 32 18 Q 32 27 17.5 27" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
            {[[16,9],[32,18],[17.5,27],[16,41]].map(([x,y],i)=>(<circle key={i} cx={x} cy={y} r="2.6" fill="var(--emerald-500)" />))}
            <circle cx="42" cy="18" r="3" fill="var(--emerald-400)" />
          </g>
        </svg>
      </Frame>
    );
        ...
        <svg viewBox="0 0 420 520" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" aria-hidden="true" style={{ display: 'block' }}>
          <defs>
            <radialGradient id={g + '-wash'} cx="0.35" cy="0.3" r="0.85">
              ...
              <stop offset="0.5" stopColor="rgba(37,99,235,0.08)" />
              <stop offset="1" stopColor="rgba(6,11,24,0)" />
            </radialGradient>
          ...
          {nodes.map((n, i) => n.on ? (
            <g key={i} className="node-pulse" style={{ ['--d']: `${(i % 5) * 0.6}s`, transformOrigin: `${n.x}px ${n.y}px` }}>
              <circle cx={n.x} cy={n.y} r={n.r + 5} fill="rgba(37,99,235,0.12)" />
        ...
        <svg viewBox="0 0 960 400" width="100%" height="100%" preserveAspectRatio="xMidYMid slice" aria-hidden="true" style={{ display: 'block' }}>
          <defs>
            ...
            <linearGradient id={g + '-r2'} x1="0" y1="0" x2="1" y2="0">
              ...
              <stop offset="0.5" stopColor="rgba(37,99,235,0.55)" />
