  ...
  function ArticleCard({ article, readMore }) {
    return (
      <GlassCard pad={0} style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', height: '100%' }}>
        <div style={{ height: 150, background: 'linear-gradient(135deg, rgba(45,107,255,0.22), rgba(37,99,235,0.14))', borderBottom: '1px solid var(--glass-border)', position: 'relative' }}>
  ...
  function CtaBand({ t, go }) {
    return (
      <GlassCard strong pad={'clamp(32px, 5vw, 56px)'} style={{
        ...
        <div aria-hidden="true" style={{ position: 'absolute', inset: 0, background: 'radial-gradient(600px 240px at 50% -10%, rgba(37,99,235,0.18), transparent 70%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ marginTop: 6 }}>
            <Button variant="primary" onClick={() => go('contact')}>{t.cta} <Icon name="arrow-right" size={17} color="#FFFFFF" /></Button>
          </div>
        </div>
      </GlassCard>
    );
